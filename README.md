...
The tables below show some example that users can follow. They demonstrate the full HERMES regression workflow: generating longitudinal data using a known process via `hermes-run`, recovering the underlying coefficients using `hermes-regress`, then applying the fitted model in a new microsimulation.
...
| 2️⃣  **Train regression model**                           | `hermes-regress -u core -c time_stochastic_getclones -r linear -p income -e income --uid level_1_uid` | Run the regression engine with a linear model to produce a result that can be used later. We use a training dataset produced by HERMES here, but the dataset you use can come from any source. | A model is created in the `regressions` folder of the specified universe. The parameters are stored in a `pkl` files, and metadata in a `json` file. You should see coefficients very similar to the `TimeStochastic` transition model'. |
...
| Step                                                    | Command                                                                                                                  | Description                                                                                          | Lesson                                                                                                                                                                                                                                                                                  |
|---------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1️⃣  **Create longitudinal dataset**                      | `hermes-run -u core -c education_basic_getclones`                                                                        | Run in microsimulation mode to create a longitudinal dataset to train a regression model.            | As in the microsimulation examples, this produces a population at *N* time points, with persistent personal and household identifiers. Individuals have a probability of educational progression (the `education` domain) determined by their age, income and existing education level. |
| 2️⃣  **Train regression model**                           | `hermes-regress -u core -c education_basic_getclones -r logistic -p age income education -e education --uid level_1_uid` | Run the regression engine with a logistic model to produce a result that can be used later.          | A model is created in the `regressions` folder of the specified universe. The parameters are stored in a `pkl` files, and metadata in a `json` file. You should see coefficients very similar to the `EducationLogit` transition model.                                                 |
| 3️⃣  **Apply regression model in microsimulation**        | `hermes-run -u core -c education_basic_regression`                                                                       | The regression model computed in the previous steps is applied to the `education` domain at runtime. | You should see an advancement rate in the `education` domain that matches the ground-truth value specified in the first step.                                                                                                                                                           |
